...
if(config.debug) console.log('[Startup] Authentication loaded');

// Register
exports.localReg = function (username, password, callback) {
	if(config.debug) console.log('[Passport] register called');
	...
	db.serialize(function() {
		...
		db.each("SELECT nickname FROM accounts WHERE nickname = (?)", username, function(err, row) {
			if (err) throw err;
			if(config.debug) console.log("[Passport] Username, "+username+", already exists.");
			existing = true;
		}, (function(err, numberOfRows){
			// callback after .each
			if(config.debug) console.log("[Passport] Callback after .each started. Number of rows: " + numberOfRows);
			...
			if(!existing){
				...
				db.run("INSERT INTO accounts (nickname, password) VALUES (?,?)", [username, hash], function(err){
					...
					if (err) throw err;
					if(config.debug) console.log('[Passport] '+username + ' added to the database! New player, yay!');
				});
...
exports.localAuth = function (username, password, callback) {
	if(config.debug) console.log('[Passport] login called');
	var	user = false;
	var err = null;
	...
	db.serialize(function() {
		db.each("SELECT id, nickname, password FROM accounts WHERE nickname = (?)", username, function(err, row) {
			...
			if(bcrypt.compareSync(password, row.password)){
				// there is a mach!
				if(config.debug) console.log('[Passport] ' +username + ' has good credentials!');
				user = {
						"id": row.id,
						"username": row.nickname,
						"currentGame": null,
				...
				if(config.debug) console.log('[Passport] ' +username + ' tried to log in, but failed. Bad credentials.');
			}
